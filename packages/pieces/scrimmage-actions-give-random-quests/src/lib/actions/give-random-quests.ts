...
export const giveRandomQuests = createAction({
  name: 'give_random_quests', // Must be a unique across the piece, this shouldn't be changed.
  auth: PieceAuth.None(),
  ...
  async run(context) {
    if (context.executionType === ExecutionType.BEGIN) {
      const response = await scrimmageCommon.giveRandomQuestsToUser(context.propsValue['user_id'], {
    ...
    const succeededQuests = response.quests.filter(quest => quest.success);
      await context.store?.put('_scrimmage_give_random_quests_ids', {
      ...
      if (succeededQuests.length > 0 && context.propsValue['wait_till'] !== 'immediately') {
        ...
        const responseOnQuestCompleted = await scrimmageCommon.subscribeWebhook('game.event', 'QUEST_COMPLETED', resumeLink, 'GET')
        console.log(responseOnQuestCompleted);
        const responseOnQuestCanceled = await scrimmageCommon.subscribeWebhook('game.event', 'QUEST_CANCELED', resumeLink, 'GET')
        console.log(responseOnQuestCanceled);
        const responseOnQuestLost = await scrimmageCommon.subscribeWebhook('game.event', 'QUEST_LOST', resumeLink, 'GET')
    ...
    try {
      ...
      const questIds = await context.store?.get<any>('_scrimmage_give_random_quests_ids');

      if (questIds.ids.includes(parsedAction.target.id)) {
        if (context.propsValue['wait_till'] === 'on_any_quest_completed') {
          await context.store?.delete('_scrimmage_give_random_quests_ids')
          return {
            approved: true,
          }
        }
